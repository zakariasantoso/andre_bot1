...
// ANTI LINK GRUP
	    ...
	    if (authorname != undefined) { } else { authorname = groupName }				function addMetadata(packname, author) {	
				...
				if (!packname) packname = 'BABYBOT'; if (!author) author = 'RudeSploit';	
...
*/
			...
			break
				...
				if (!isRegistered) return reply(nad.noregis())
				if (isLimit(sender)) return reply(nad.limitend(pusname))
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis Zakaria baik hati:v`)
				nul = body.slice(7)
				...
				if (args.length < 1) return reply(`Teksnya mana kak? Contoh : ${prefix}nulis1 Zakaria baik hati`)
